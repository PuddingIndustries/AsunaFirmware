...
static const char *LOG_TAG = "N_COMMON";
...
static void app_netif_ip_event_cb(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (event_id == IP_EVENT_PPP_GOT_IP) {
        ...
        ESP_LOGI(LOG_TAG, "Modem Connect to PPP Server");
        ESP_LOGI(LOG_TAG, "~~~~~~~~~~~~~~");
        ...
        ESP_LOGI(LOG_TAG, "Name Server1: " IPSTR, IP2STR(&dns_info.ip.u_addr.ip4));
        esp_netif_get_dns_info(netif, 1, &dns_info);
        ESP_LOGI(LOG_TAG, "Name Server2: " IPSTR, IP2STR(&dns_info.ip.u_addr.ip4));
        ESP_LOGI(LOG_TAG, "~~~~~~~~~~~~~~");
        ESP_LOGI(LOG_TAG, "GOT ip event!!!");
    } else if (event_id == IP_EVENT_PPP_LOST_IP) {
        ESP_LOGI(LOG_TAG, "Modem Disconnect from PPP Server");
    } else if (event_id == IP_EVENT_GOT_IP6) {
        ESP_LOGI(LOG_TAG, "GOT IPv6 event!");
